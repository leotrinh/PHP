    ...
    <div class="tk" >
		...
		$r = $db->select("tgp_cms","cat = '".$row2["cat"]."'  and hien_thi = 1 AND id <> ".$id,"order by time desc limit 5");
          ...
          while ($row = $db->fetch($r))
          {
			...
			<a data-tooltip2="sticky_<?=$row["id"]?>" href="/tin-tuc-xem/<?=$row["id"]?>/<?=(lg_string::get_link(lg_string::crop($row["ten"],20)))?>"><font color="#FAD378">&raquo; &nbsp;<?=lg_string::crop($row["ten"],30)?></font></a><a class="dang_ngay" style="font-style:italic;"> (<?=lg_date::vn_date($row["time"])?>)</a>
			</div>
