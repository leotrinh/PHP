...
<?php
	...
	if ($func == "new")
	{
		...
		{
			$db->insert("tgp_cms_menu","id,cat,ten,thu_tu,hien_thi,type,noi_bat,rss_link","0,'".$db->escape($txt_cat)."','".$db->escape($txt_ten)."','".(cat_count($txt_cat)+1)."','".($txt_hien_thi+0)."','".($txt_type+0)."','".($txt_noi_bat+0)."','".$db->escape($txt_rss_link)."'");
	...
	if (!$OK)
		template_edit("?act=cms_menu_new","new",0,$txt_cat,$txt_ten,$txt_hien_thi,$txt_type,$txt_noi_bat,$txt_rss_link,$error);
?>
</center>