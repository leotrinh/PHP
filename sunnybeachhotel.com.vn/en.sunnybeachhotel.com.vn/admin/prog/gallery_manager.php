...
<div class="function">
	<a href="?act=gallery_new"><img src="images/add_new.gif" align="absmiddle" border="0" /></a> <a href="?act=gallery_new">Thêm hình mới</a>
</div>
<?php
	$delete = $delete + 0;
	if ($delete != 0)
	{
		...
		admin_load("Đã xóa thành công.","?act=gallery_manager");
	}
	if ($func == "sort")
	{
		$r	=	$db->select("tgp_cat");
		...
		admin_load("Đã sắp xếp thành công.","?act=gallery_manager");
	}
?>
...
?>
  <tr class="tb_foot">
	...
	<td>-</td>
    <td><a href="?act=gallery_menu_new&txt_cat=<?=$row["id"]?>"><img src="images/i_add.gif" border="0" /></a></td>
    <td>-</td>
    <td>-</td>
    <td>-</td>
  </tr>
<?php
	$r2	=	$db->select("tgp_gallery_menu","cat = '".$row["id"]."'","order by thu_tu asc");
...
?>
  <tr class="tb_content">
    ...
    <td><a href="?act=gallery_new&txt_cat=<?=$row2["id"]?>">Thêm hình</a></td>
    <td><a href="?act=gallery_menu_edit&id=<?=$row2["id"]?>">Sửa</a></td>
    <td><a href="?act=gallery_manager&delete=<?=$row2["id"]?>" onclick="return confirm('Tất cả bài viết sẽ bị mất hết\nBạn có chắc chắn không ?');">Xóa</a></td>
    <td><a href="?act=gallery_list&id=<?=$row2["id"]?>"><img src="images/go_right.gif" border="0" /></a></td>
  </tr>
<?php
	}
}
