	...
	if ($func == "update")
	{
		...
		else
		{
			...
			if ($OK)
			{
				...
				$db->query("update tgp_cms set cat = '".$db->escape($txt_cat)."', ten = '".$db->escape($txt_ten)."', chu_thich = '".$txt_chu_thich."', hinh_note = '".$db->escape($txt_hinh_note)."', noi_dung = '".$txt_noi_dung."', hien_thi = '".($txt_hien_thi+0)."', noi_bat = '".($txt_noi_bat+0)."', time = '".$time."' where id = '".$id."'");
				if ($hinh)
				{
					
					$txt_hinh_1	= "cms_".$id.".".$file_type;
					img_resize($up_dir.$file_full_name,$up_dir.$txt_hinh_1,"w=140&h=100&zc=1");
						$txt_hinh_4	= "thumb_".$id.".".$file_type;
					img_resize($up_dir.$file_full_name,$up_dir.$txt_hinh_4,"w=515&h=350&zc=1");
					$txt_hinh_6	= $id.".".$file_type;
						img_resize($up_dir.$file_full_name,$up_dir.$txt_hinh_6,"w=10&h=10&zc=1");
					
					$db->update("tgp_cms","hinh",$txt_hinh_6,"id = '".$id."'");
				}
