	...
	if ($func == "new")
	{
		...
		else
		{
			...
			if ($OK)
			{
				...
				$id = $db->insert("tgp_cms","id,cat,ten,chu_thich,hinh_note,noi_dung,hien_thi,noi_bat,time,user","0,'".($txt_cat+0)."','".$db->escape($txt_ten)."','".$txt_chu_thich."','".$db->escape($txt_hinh_note)."','".$txt_noi_dung."','".($txt_hien_thi+0)."','".($txt_noi_bat+0)."','".$time."','".$thanh_vien["id"]."'");
